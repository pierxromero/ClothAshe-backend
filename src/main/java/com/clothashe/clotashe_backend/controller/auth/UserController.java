...
public class UserController {
    ...
    @Operation(
            ...
            description = "Returns a list of all registered users. Access restricted to users with ADMIN role."
    )
    @ApiResponses({
    ...
    @Operation(
            ...
            description = "Retrieves a user by their ID. Requires ADMIN privileges."
    )
    @ApiResponses({
    ...
    @Operation(
            ...
            description = "Finds a user using their email address. Only accessible by ADMINs."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "User found"),
            @ApiResponse(responseCode = "400", description = "Invalid email format", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "404", description = "User not found", content = @Content(schema = @Schema(implementation = ApiError.class)))
    })
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/search")
    public ResponseEntity<UserDTO> getByEmail(@RequestParam @Parameter(description = "Email address of the user") String email) {
        return ResponseEntity.ok(userService.getByEmail(email));
    }

    @Operation(
            summary = "List locked users",
            description = "Returns all users currently locked. Accessible by ADMIN role only."
    )
    @ApiResponses({
    ...
    @Operation(
            summary = "Delete user",
            description = "Deletes a user by ID. Only users with ADMIN role are authorized."
    )
    @ApiResponses({
    ...
    @Operation(
            ...
            description = "Modifies a user's role. Requires OWNER role."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Role updated successfully"),
            @ApiResponse(responseCode = "400", description = "Invalid role provided", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "403", description = "Forbidden - Only OWNER can perform this action", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "404", description = "User not found", content = @Content(schema = @Schema(implementation = ApiError.class)))
    })
    @PreAuthorize("hasRole('OWNER')")
    @PatchMapping("/{id}/role")
    public ResponseEntity<UserDTO> changeUserRole(@PathVariable Long id, @Valid @RequestBody RoleChangeDTO dto) {
        return ResponseEntity.ok(userService.changeUserRole(id, dto));
    }

    @Operation(
            summary = "Lock/unlock user",
            description = "Locks or unlocks a user account. Requires ADMIN access."
    )
    @ApiResponses({
    ...
    public ResponseEntity<UserWithAuthInfoDTO> lockUnlockUser(@PathVariable Long id, @Valid @RequestBody LockUnlockDTO dto) {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Invalid password format or mismatch", content = @Content(schema = @Schema(implementation = ApiError.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(schema = @Schema(implementation = ApiError.class)))
    })
    @PreAuthorize("isAuthenticated()")
    ...
    @Operation(
            ...
            description = "Deletes the account of the authenticated user permanently."
    )
    @ApiResponses({
